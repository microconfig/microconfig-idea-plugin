...
public class ResolvePlaceholderValue extends ResolvePlaceholderBase implements MicroconfigComponent {
    ...
    @Override
    public void react() {
        String placeholder = placeholderSubstring(currentLine, context.getCaret().getLogicalPosition().column)
                .orElseThrow(() -> new IllegalArgumentException("Not inside placeholder brackets"));

        //todo error hint if special placeholder
        Map<String, String> values = api.resolveOnePlaceholderForEachEnv(placeholder, context.currentFile(), context.projectDir());
        printValues(values);
    }
}
